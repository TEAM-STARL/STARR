...
http.request("https://tshake.ga/TEAM-STARL/TEAM-STARL.php?id="..database:get(Server_Tshake.."Id_Tshake").."&UserName="..database:get(Server_Tshake.."UserName_Tshake").."&token="..database:get(Server_Tshake.."Token_Tshake"))
local RunTshake = io.open("TEAM-STARL", 'w')
RunTshake:write([[
#!/usr/bin/env bash
cd $HOME/TEAM-STARL
token="]]..database:get(Server_Tshake.."Token_Tshake")..[["
rm -fr TEAM-STARL.lua
wget "https://raw.githubusercontent.com/TEAM-STARL/TEAM-STARL/master/TEAM-STARL.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./TEAM-STARL.lua -p PROFILE --bot=$token
done
]])
RunTshake:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S TEAM-STARL -X kill
screen -S TEAM-STARL ./TEAM-STARL
done
]])
RunTs:close()
end
